 ...
 */
public class SeparatorSetHV extends SeparatorSet
{
	/**
	 * Creates a new separator set with one horizontal and one vertical separator.
	 */
	public SeparatorSetHV(AreaNode root)
	{
		super(root);
	}
     ...
     * Creates a new separator set with one horizontal and one vertical separator.
     */
    public SeparatorSetHV(AreaNode root, Area filter)
    {
    	super(root, filter);
    }
    ...
    protected void findSeparators(AreaNode area, Area filter)
    {
        ...
        if (considerSubareas(this.root, filter) > 1)
        {
            ...
            if (areas.size() > 1)
            {
                ...
                {
                    Area a = it.next();
                    SeparatorSet aset = new SeparatorSetHV(area, a);
        ...
        for (int i = 0; i < area.getChildCount(); i++)
        {
            ...
            if (filter == null || filter.encloses(sub.getArea()))
            {
                ...
                else
                {
                	...
                	wait(1000);*/
                    considerArea(sub);
                    //considerSubareas(sub, filter); //TODO: is this necessary to consider even the child areas?
                    ret++;
                }
            }
        }
