...
public class TreeTagger implements NodeJoinAnalyzer
{
    ...
    //public static AbstractSequenceClassifier<?> sharedClassifier = CRFClassifier.getJarClassifier("3class.gz", null);
    ...
    public TreeTagger(AreaTree tree)
    {
        this.tree = tree;
     ...
     */
    public void tagSubtree(AreaNode root)
    {
        tagSingleNode(root);
     ...
     */
    public void tagSingleNode(AreaNode area)
    {
        for (Tagger t : taggers)
        {
            if (t.belongsTo(area))
                area.addTag(t.getTag());
        }
    }
